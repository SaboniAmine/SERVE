    ...
    def read_file_and_extract_votes(self, init_file: Any):
        ...
        for minutes, amendment_ids in minutes_amendment_ids.items():
            try:
                ...
                exceptions.append(f"Can't extract votes for minutes {minutes.id} and amendment_ids {amendment_ids} ({e})")
        ...
        for id, row in df_init.iterrows():
            try:
                if pd.notna(row.ID):
                    ...
                    minutes = self.convert_minutes_from_pdf(minutes_id, row.Subject_Sarah, minutes_pdf)
                    ...
                    if len(amendment_id_to_extract) == 0:
                        logger.warning(f'No amendment found for row {id}: p.{row.Page} - {row.ID}, check the specified page number.')
                    else:
