...
class MinutesAggregate(BaseModel):
    ...
    def extract_votes_from_amendments(minutes: Minutes, amendment_ids: List[str]) -> List[Vote]:
        ...
        for amendment in amendments:
            ...
            # for every vote category, extract the MEP
            regex_group = r"\n+?(.+?): *"
            for i in range(0, len(votes_by_category), 3):
                ...
                category_votes = votes_by_category[i + 2]
                votes_by_group = re.split(regex_group, category_votes)
                extracted_category_votes = 0
                for i in range(1, len(votes_by_group), 2):
                    current_group_short_name = votes_by_group[i]
                    ...
                    for name in group_voters:
                        ...
                        extracted_votes.append(Vote(amendment_id=amendment.id, value=vote_category, mep=mep))
